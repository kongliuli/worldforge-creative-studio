...
| 接口 | 实现 | 方法 |
|------|------|------|
| `IProjectService` | Infrastructure | ListRecent, Get, Create, UpdateSettings（AutoDetect / Ollama 模型） |
| `IContradictionService` | Infrastructure | DetectAsync（含可选 `focusDocumentId`） |
| `IContradictionQueryService` | Infrastructure | List（status/severity 过滤）、UpdateStatus ✅ v2 |
| `IManuscriptService` | Infrastructure | GetTree, CRUD, Reorder ✅ 2026-07-05 |
| `IEntityService` | Infrastructure | CRUD + 前缀过滤（并入 `ListAsync(discriminator, prefix)`） ✅ 2026-07-05 |
| `IImportService` | Infrastructure | ZIP（Markdown / Scrivener）导入 ✅ v1 |

### 4.2 规划接口与已落地差异

与 [07 §七](./07-服务端API.md) API 一一对应；**接口在 Core，实现在 Infrastructure**。

> ⚠️ 本节草案签名与已落地代码存在漂移，**以 `Core/Services/*.cs` 为准**。已知差异：`SaveAsync`/`DeleteAsync`/`ReorderAsync` 返回 `Task<bool>`；`CreateAsync` 带 `int? sortOrder`；`CreateEntityRequest` 无 `Fields` 字典；前缀搜索并入 `IEntityService.ListAsync(..., prefix)` 而非独立 `SearchByPrefixAsync`。仍未实现的仅剩 `INoteQueryService` 与 `IRelationshipService`。

#### INoteQueryService — 通用 Note 读（规划 · 未实现）
...
#### IEntityService — 租户实体 CRUD ✅ 已实现（签名以 `IEntityService.cs` 为准）
...
#### IContradictionQueryService — 矛盾列表 ✅ 已实现（v2）
